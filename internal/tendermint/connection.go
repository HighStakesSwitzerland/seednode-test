...
func (c *MConnection) flush() {
	...
	if err != nil {
		logger.Debug("MConnection flush failed", "err", err)
	}
}
...
func (c *MConnection) _recover() {
	if r := recover(); r != nil {
		logger.Error("MConnection panicked", "err", r, "stack", string(debug.Stack()))
...
func (c *MConnection) stopForError(r interface{}) {
	if err := c.Stop(); err != nil {
		logger.Error("Error stopping connection", "err", err)
	}
...
func (c *MConnection) Send(chID byte, msgBytes []byte) bool {
	...
	if !ok {
		logger.Error(fmt.Sprintf("Cannot send bytes, unknown channel %X", chID))
		return false
	}
	...
	if success {
		...
		logger.Debug("Send failed", "channel", chID, "conn", c, "msgBytes", fmt.Sprintf("%X", msgBytes))
	}
	return success
}
...
func (c *MConnection) CanSend(chID byte) bool {
	...
	channel, ok := c.channelsIdx[chID]
	if !ok {
		logger.Error(fmt.Sprintf("Unknown channel %X", chID))
		return false
	}
...
FOR_LOOP:
	...
	SELECTION:
		...
		case <-c.pingTimer.C:
			...
			if err != nil {
				logger.Error("Failed to send PacketPing", "err", err)
				break SELECTION
			}
		...
		case <-c.pong:
			...
			if err != nil {
				logger.Error("Failed to send PacketPong", "err", err)
				break SELECTION
			}
			c.sendMonitor.Update(_n)
			c.flush()
		...
		*/
		...
		case *tmp2p.Packet_PacketMsg:
			...
			if pkt.PacketMsg.ChannelID < 0 || pkt.PacketMsg.ChannelID > math.MaxUint8 || !ok || channel == nil {
				...
				logger.Debug("Connection failed @ recvRoutine", "conn", c, "err", err)
				c.stopForError(err)
				break FOR_LOOP
			}
